...
public class JDBCExample {
	public static void main(String[] args) {
		...
		try {
			...
			// 1. DB 연결에 필요한 Oracle JDBC Driver에 로드하기
												// -> 객체로 만들어두기
			
			Class.forName("oracle.jdbc.driver.OracleDriver");
			// -> () 안에 작성된 클래스의 객체를 반환
			// -> 메모리에 객체가 생성되어지고 JDBC 필요시 알아서 참조해서 사용
			// -> 생략해도 자동으로 메모리 로드가 진행됨(명시적으로 작성하는걸 권장)
			
			// 2. 연결 정보를 담은 Connection을 생성
			// -> DriveManager 객체를 이용해서 Connection 객체를 만들어 얻어옴!
			...
			// DriveManager : 메모리에 로드된 JDBC 드라이버를 이용해서 
			...
			// 3. SQL 작성
			// ** Java에서 작성되는 SQL은 마지막에 ;(세미콜론)을 찍지 않아야한다! **
			...
			while(rs.next()) { 
				...
				// -> java.util.Date도 가능하긴 함(부모클래스여서 다형성 적용)
