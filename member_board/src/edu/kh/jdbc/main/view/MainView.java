			...
			try {
				...
				if (loginMember == null) {
					...
					case 0 : 
						loginMember = null;
						System.out.println("\n[로그아웃 되었습니다.]\n");
						input = -1; // do-while문 종료되지 않도록 0이 아닌 값으로 변경
						break; 
					// 로그아웃 == loginMember 참조하는 객체 없음(==null)
					// 로그인 == loginMember 참조하는 객체 존재
					case 99 :
	...
	private void signUp() {
		...
		try {
			...
			while (true) {
				...
				memberGender = sc.next().toUpperCase(); // 입력받자마자 대문자 변경
				/*
				 * if(memberGender.equals("M")) { System.out.println("남성"); break; } else
				 * if(memberGender.equals("F")) { System.out.println("여성"); break; } else {
