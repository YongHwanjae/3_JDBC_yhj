...
public class BoardView {
	...
	private void subBoardMenu(Board board) {
		...
		try {
			...
			System.out.println("\n<< 입력 형식이 올바르지 않습니다.>>");
			sc.nextLine(); //입력 버퍼에 남아있는 뭊자열 제거
		}
		...
		private void insertComment(int bNo, int mNo) {
			...
			try {
				...
				// DB INSERT시 필요한 값을 하나의 Comment 객체의 저장
	...
	private void updateComment(List<Comment> commentList, int memberNo) {
		...
		try {
			...
			for(Comment c : commentList) {
				...
				if(c.getCommentNo() == commentNo) { // 댓글번호 일치
					flag = false;
			...
			System.out.println("/n 댓글 수정 중 예외 발생");
	...
	private void deleteBoard(int boardNo) {
		try {
			...
			char ch = sc.next().toLowerCase().charAt(0); // 소문자로 변환해서 첫 글자
